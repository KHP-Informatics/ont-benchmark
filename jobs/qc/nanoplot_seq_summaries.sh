...
for summary_file in /scratch/prj/ppn_als_longread/ont-benchmark/seq_summaries/*/*.txt; do
    if [ -f "${summary_file}" ]; then
        subdir_name=$(basename "$(dirname "${summary_file}")")
        nanoplot_outdir="/scratch/prj/ppn_als_longread/ont-benchmark/qc/nanoplot/seq_summaries/${subdir_name}"

        summary_files+=("${summary_file}")
        summary_names+=("${subdir_name}")

        if [[ "${subdir_name}" == *__* ]]; then
            srun -n1 -N1 singularity exec --bind /scratch docker://quay.io/biocontainers/nanoplot:1.42.0--pyhdfd78af_0 \
                NanoPlot \
...
nanocomp_outdir="/scratch/prj/ppn_als_longread/ont-benchmark/qc/nanocomp/seq_summaries/"
...
if [ ${#summary_files[@]} -gt 0 ]; then
    ...
    srun -n1 -N1 singularity exec --bind /scratch docker://quay.io/biocontainers/nanocomp:1.23.1--pyhdfd78af_0 \
        NanoComp \
